        ...
        _this.refactorMapRequestToState = function (mappings) {
          ...
          Object.keys(mappings).forEach(function (propName) {
            ...
            if (Function.prototype.isPrototypeOf(mapRequest)) {
              ...
              // 添加至懒加载队列
              lazyRequest.push(_this.mapRequestByType(propName, mapRequest, true));
              // 初始化responses
              responses[propName] = {
                status: 'pending',
                loading: true,
        ...
        _this.recursionRequest = function () {
          ...
          return Promise.all(childRequests.map(function (item, index) {
            return item.request().then(function (response) {
              ...
              if (item.then) {
                var mixRequest = _this.buildRequestByFunctionCall(item.then(response.data));
        ...
        _this.mapRequestByType = function (propName, mapRequest) {
          ...
          if (typeof mapRequest === "string") {
            ...
            return {
              ...
              request: _this.makeRequest(_extends({}, mapRequest, {
                isLazy: isLazy
              }))
            };
          } else {
        ...
        _this.makeRequest = function () {
          ...
          return function () {
            var url = options.url,
                ...
                otherOptions = _objectWithoutProperties(options, ['url', 'mapResult', 'successText']);

            return new Promise(function (resolve, reject) {
              (0, _middleware.compose)('before')([options], function () {
                (0, _helper.buildFetch)(url, otherOptions).then(function (result) {
                  return {
                    status: 'success',
                    loading: false,
                    error: false,
